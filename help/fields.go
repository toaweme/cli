...
// isPositionalArg reports whether an arg tag names a positional argument
// (a bare index like "0"), as opposed to a named flag.
...
// commandDescription returns the command's long-form description with trailing newlines trimmed.
...
// carries a oneof rule, so listings and tables show the permitted values. extra adds
// dynamically discovered values (e.g. output codecs registered for --help-format)
...
// formatHintExtras returns extraFormats only for the global --help-format field,
// so the dynamic format names ride along on that flag's allowed-values hint and nowhere else.
...
// displayType renders a field's type for help output, preferring the concrete Go type
// for slices ("[]string") over the bare reflect kind ("slice").
...
// globalSource returns the global flags struct to render the Global Options block from:
// the populated one when provided (so --help-values shows set values),
// else a zero struct (just the flag definitions).
